/*
   ...
   https://github.com/BalazsJako/ImGuiColorTextEdit and slightly modified to
   work with modern ImGui versions. All credits go to the original author. Show
   them some love!
*/
...
TextEditor::TextEditor()
    : mLineSpacing(1.0f), mUndoIndex(0), mTabSize(4), mOverwrite(false),
      ...
      mSelectionMode(SelectionMode::Normal), mHandleKeyboardInputs(true),
      mHandleMouseInputs(true), mIgnoreImGuiChild(false),
      mShowWhitespaces(true), mCheckComments(true),
  ...
  if (lineNo >= 0 && lineNo < (int)mLines.size()) {
    ...
    while ((size_t)columnIndex < line.size()) {
      ...
      if (line[columnIndex].mChar == '\t') {
        ...
        int i = 0;
        while (i < 6 && d-- > 0 && columnIndex < (int)line.size())
...
std::string TextEditor::GetWordAt(const Coordinates &aCoords) const {
  ...
  for (auto it = istart; it < iend && it < (int)line.size(); ++it)
...
void TextEditor::HandleKeyboardInputs() {
  ...
  if (ImGui::IsWindowFocused()) {
    ...
    if (!IsReadOnly() && ctrl && !shift && !alt &&
      ...
      Delete();
    // TODO: make it so that ctrl + backspace deletes the previous word? (like
...
float TextEditor::TextDistanceToLineStart(const Coordinates &aFrom) const {
  ...
  float spaceSize = ImGui::GetFont()
                        ->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f,
                                        " ", nullptr, nullptr)
                        .x;
  size_t colIndex = GetCharacterIndex(aFrom);
