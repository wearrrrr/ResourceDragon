...
if not _port then   -- invalid seek
  ...
  elseif os.getenv("LUA_PLATFORM") == "Windows" then
    -- seeking works for stdin on Windows
...
if not _port then
  ...
  do  -- basic tests for popen
    ...
    local f
    if os.getenv("LUA_PLATFORM") == "POSIX" then
      f = assert(io.popen("cat - > " .. file, "w"))
    elseif os.getenv("LUA_PLATFORM") == "Windows" then
      f = assert(io.popen("findstr \"^\" > " .. file, "w"))
    end
    f:write("a line")
    assert(f:close())
    if os.getenv("LUA_PLATFORM") == "POSIX" then
    ...
    elseif os.getenv("LUA_PLATFORM") == "Windows" then
      f = assert(io.popen("@echo off&&type " .. file, "r"))
    end
    local line = f:read("a")
if os.getenv("LUA_PLATFORM") == "POSIX" then
  assert(line == "a line")
elseif os.getenv("LUA_PLATFORM") == "Windows" then
  ...
  if os.getenv("LUA_PLATFORM") == "POSIX" then
    tests = {
      ...
      -- Run additional actions after the kill command to avoid direct execve within the shell
      -- (/bin/sh shell optimisation on FreeBSD)
  ...
  elseif os.getenv("LUA_PLATFORM") == "Windows" then
    tests = {
      ...
      {"exit 3", "exit", 3},
      {"exit 129", "exit", 129},
      -- {progname .. ' -e " "', "ok"}, -- quoting a space on Windows with cmd.exe is difficult
      {progname .. ' -e os.exit(0,true)', "ok"},
      {progname .. ' -e os.exit(20,true)', "exit", 20},
    }
  end
...
if not _port then
  ...
  if os.getenv("LUA_PLATFORM") == "POSIX" then
    ...
    print(string.format("t0=%s, t1=%s", t0, t1))
    assert(t1 - t0 == -(1 << 31))
  elseif os.getenv("LUA_PLATFORM") == "Windows" then
    ...
    print(string.format("t0=%s, t1=%s", t0, t1))
    assert(t1 - t0 == (1 << 31) - 1)
    t0 = os.time{year = 1970, month = 1, day = 1, sec = -43200}
    assert(t0 == 0)
  end
      ...
      if testerr("out-of-bound", os.date, "%Y", 2^40) then
        ...
        if os.getenv("LUA_PLATFORM") == "POSIX" then
          assert(tonumber(os.time
            {year=(1 << 30) + 1073741823, month=12, day=31, hour=23, min=59, sec=59}))
        elseif os.getenv("LUA_PLATFORM") == "Windows" then
          assert(tonumber(os.time
        ...
        elseif os.getenv("LUA_PLATFORM") == "Windows" then
          checkerr("represented", os.time,
            {year=3001, month=1, day=19, hour=7, min=59, sec=60})
        end
      end
