...
local LDIR = P(os.getenv("LUA_TEST_LIBRARY_PATH"))
local LRDIR = P(os.getenv("LUA_RELATIVE_TEST_LIBRARY_PATH"))
...
local function DC (x)
  local ext = (dirsep == '\\') and (LPOSTFIX .. ".dll") or ".so"
  return LD(x .. ext)
end
...
if not st then
  ...
  assert(string.find(ext, P(LRDIR.."lib2-v2"..LPOSTFIX), 1, true))
  ...
  assert(string.find(ext, P(LRDIR.."lib1"..LPOSTFIX), 1, true))
  assert(fs.id(45) == 45)
  _ENV.x, _ENV.y = nil
end
